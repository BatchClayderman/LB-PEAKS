...
def Encrypt(pars:PARS) -> PARS:
	...
	A_kw = (np_sum(kw[:, None, None] * vec_M, axis = 0) % q + B) % q # size = (n, 2m)
	...
	R_kw = np_sum(kw[:, None, None] * R, axis = 0) % q # size = (2m, 2m)
	noi = randint(q)
...
def Trapdoor(pars:PARS) -> PARS:
	...
	T_A_ID = ExtBasis(T_A, A_ID) # size = (2m, 2m)
	trap_1 = dot(SK_ID, y) % q # size = (m, 1)
	trap_2 = SampleLeft(A_ID, A_kw, T_A_ID, U, q) # size = (4m, 1)
...
def Test(pars:PARS) -> bool:
	...
	sigma = Trap[2]
	h = sigma[0]
	print(int((h == H_2(concatenate(((A_z - H_1(ID, q, n) * h % q) % q, trap_1), axis = 1))).all() and matrix_rank(Z) <= (s << 1) * m ** 0.5 and matrix_rank((c_0 - dot(trap_2.T, c_1) % q) % q) <= q >> 2))
	print()
	return bRet
